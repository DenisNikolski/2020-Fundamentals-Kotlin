...
object KotlinWorkshop3 {
    ...
    private fun playRound(userInput: Int, randomNumber: Int): Boolean {
        ...
        when {
            ...
            userInput < randomNumber -> {
                msg = "Your Guess is Lower"
            }
            else -> {
                msg = "Your Guess is Higher"
            }
        }
    ...
    private fun printGameStats(guesses: IntArray, guessCounter: Int, randomNumber: Int) {
        ...
        guesses.forEach { guess -> if (guess != -1) println(guess) }
    }
    ...
    private fun IntArray.countHigherGuesses(guesses: IntArray, randomNumber: Int): Int {
        var higherGuessesCount: Int = 0
        guesses.forEach() { guess ->
            if (guess > randomNumber) {
                println("higher guess: $guess")
                higherGuessesCount++
            }
        }
        println("higher Guesses Count: $higherGuessesCount")
        return higherGuessesCount
    }
    ...
    val countLowerGuesses: (IntArray, Int) -> Unit = { guesses, randomNumber ->
        ...
        println("lower Guesses Count: $lowerGuessesCount")
    }
